...
// IWYU pragma: private, include "Units/Simulation/RigUnit_AlphaInterp.h"
...
#define FRigUnit_AlphaInterpQuat_Execute() \
	void FRigUnit_AlphaInterpQuat::StaticExecute( \
		const FRigVMExecuteContext& RigVMExecuteContext, \
		const FQuat& Value, \
		const float Scale, \
		const float Bias, \
...
#define FID_Engine_Plugins_Animation_ControlRig_Source_ControlRig_Private_Units_Simulation_RigUnit_AlphaInterp_h_161_GENERATED_BODY \
	friend struct Z_Construct_UScriptStruct_FRigUnit_AlphaInterpQuat_Statics; \
	static class UScriptStruct* StaticStruct(); \
	static void StaticExecute( \
		const FRigVMExecuteContext& RigVMExecuteContext, \
		const FQuat& Value, \
...
template<> CONTROLRIG_API UScriptStruct* StaticStruct<struct FRigUnit_AlphaInterpQuat>();
